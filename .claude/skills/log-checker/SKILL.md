...
Перед использованием проверь, что утилита установлена:

```bash
fast-log-check --version
```

Если команда не найдена — следуй инструкциям в [INSTALL.md](./INSTALL.md).
...
Для получения полного списка параметров и их описаний выполни:

```bash
fast-log-check --help
```

**Ключевые параметры для AI-интеграции:**

- `--ai-output`: **Обязательный флаг** для получения структурированного JSON. При использовании этого флага HTML-отчеты не генерируются.
- `--ai-format {compact,standard,full}`: Формат JSON вывода (`full` рекомендуется для максимальной детализации).
