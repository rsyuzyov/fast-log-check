...
```json
{
    "подстрока_для_поиска": "Текст, который будет отображаться в заголовке группы",
    "CS_ERR_LIBRARY": "Ошибки подключения к corosync",
    "can't initialize service": "Ошибка инициализации сервиса"
}
```

Если в сообщении лога будет найдена указанная подстрока, оно будет сгруппировано с другими такими же сообщениями под указанным заголовком.
