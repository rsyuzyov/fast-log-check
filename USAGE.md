...
6. **ZFS снимки** - автоматическая очистка (опционально)

### Proxmox

7. **Corosync кластер** - проблемы с кластером Proxmox
8. **PVE Proxy** - `/var/log/pveproxy/access.log` (HTTP ошибки)
9. **Виртуальные машины** - `qm list` (остановленные VM)
10. **Хранилища** - `pvesm status` (заполненность дисков)
